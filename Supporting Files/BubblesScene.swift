...
class BubblesScene: SIFloatingCollectionScene {
    ...
    var names: [String] {
        ...
        self.floatingNodes.forEach { (node) in
            if let bubbleNode = node as? BubbleNode, bubbleNode.state == .selected {
                nodesName.append(bubbleNode.labelNode.text!)
            }
        }
        return nodesName
    }
    ...
    private func actionForFloatingNode(_ node: SIFloatingNode!) -> SKAction {
